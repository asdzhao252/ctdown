...
(function () {
  ...
  //判断是否为手机页面
  function mobile() {
    if (GM_getValue("mobile").mode === "0" || GM_getValue("mobile").mode === "undefined") {
      var error;
      var r = confirm("使用的是手机版网页吗？ 如果是请点击确定 手机版如果不能使用请及时更新脚本");
      if (r == true) {
        var mode = {
          mode: "1"
        };
  ...
  //判断是否为电脑界面
  function pc() {
    if (GM_getValue("mobile").mode === "1" || GM_getValue("mobile").mode === "undefined") {
      var error;
      var r = confirm("使用的是电脑网页吗？如果是请点击确定");
      if (r == true) {
        var mode = {
          mode: "0"
        };
